...
router.post("/join", (req, res) => {
  db.collection("Lobbies").where('code', '==', req.query.lobbyCode).get().then((querySnapshot) => {
    ...
    if (querySnapshot.exists) {
      querySnapshot.forEach((doc) => {
        if (doc.exists) {
          ...
          // disconnect the user
          db.collection("Lobbies").doc(doc.id).collection("Players").doc(req.query.userId.toString()).update({
            disconnected: true
          }).then(() => {
            res.json({
              success: true,
